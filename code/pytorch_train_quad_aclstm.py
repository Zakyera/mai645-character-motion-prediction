...
import random
import read_bvh
from generate_training_quad_data import decode_quaternion_frame
import argparse
...
Condition_num=5
Groundtruth_num=5
In_frame_size = 175  # Quaternion frame: 3 root translation channels + 43 joints * 4 quaternion values
...
def write_quad_traindata_to_bvh(bvh_filename, quad_train_data):
    """Write quaternion training data back to BVH.

    Quaternion training data has:
    - channels 0:3 = scaled root translation
    - remaining channels = quaternions (w, x, y, z)

    We decode each quaternion frame back to raw BVH Euler channels,
    then write those raw channels using the standard BVH header.
    """
    raw_motion = np.array([decode_quaternion_frame(frame) for frame in quad_train_data])
...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False, save_bvh_motion=True):
    ...
    if(save_bvh_motion==True):
        ...
        write_quad_traindata_to_bvh(save_dance_folder+"%07d"%iteration+"_gt.bvh", gt_seq)
        write_quad_traindata_to_bvh(save_dance_folder+"%07d"%iteration+"_out.bvh", out_seq)
