...
Condition_num=5
Groundtruth_num=5
In_frame_size = 132  # Euler frame: 3 root translation channels + 129 rotation channels
...
class acLSTM(nn.Module):
    ...
    def calculate_loss(self, out_seq, groundtruth_seq):
        ...
        # channels 0:3 are scaled root translation -> MSE
        # channels 3: are Euler rotations in degrees -> angle-distance loss
        ...
        root_pred = pred[:, :, 0:3]
        root_target = target[:, :, 0:3]

        angle_pred = pred[:, :, 3:]
        angle_target = target[:, :, 3:]

        root_loss = nn.MSELoss()(root_pred, root_target)

        # Angle distance: 1 - cos(delta angle).
        # Convert degrees to radians.
        angle_delta_rad = (angle_pred - angle_target) * 3.141592653589793 / 180.0
...
def write_euler_traindata_to_bvh(bvh_filename, euler_train_data):
    """Write Euler training data back to BVH raw motion format.

    In preprocessing we scaled root translation channels 0:3 by 0.01.
    Before writing BVH, we undo that scale. Rotation channels are already
    Euler angles in degrees, so they are kept unchanged.
    """
    raw_motion = np.array(euler_train_data).copy()
    raw_motion[:, 0:3] = raw_motion[:, 0:3] / 0.01
...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False, save_bvh_motion=True):
    ...
    if(save_bvh_motion==True):
        ...
        write_euler_traindata_to_bvh(save_dance_folder+"%07d"%iteration+"_gt.bvh", gt_seq)
        write_euler_traindata_to_bvh(save_dance_folder+"%07d"%iteration+"_out.bvh", out_seq)
