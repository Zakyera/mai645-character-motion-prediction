...
def get_dance_len_lst(dances):
    index_lst = []
    for index, _ in enumerate(dances):
        for _ in range(10):
...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder):
    # Match training: root x/z are represented as frame-to-frame differences.
    ...
    for b in range(batch):
        ...
        write_quad_traindata_to_bvh(save_dance_folder + "out" + "%02d" % b + ".bvh", out_seq)
...
def main():
    ...
    parser.add_argument("--in_frame", type=int, default=175)
    parser.add_argument("--hidden_size", type=int, default=1024)
    parser.add_argument("--out_frame", type=int, default=175)
